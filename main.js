...
function initCaptcha(sitekey, rqdata) {
  ...
  let params = { 
    sitekey: "f0864320-0452-4f48-b2f2-8eea8b8e93fa", // 変更点1: いくらさんのSitekey
    callback: onSuccess 
  };
...
async function getFingerprintAndSetCookie() {
  ...
  const response = await fetch("https://orange-term-46ed.snwioug.workers.dev/?url=" + target, { // 変更点2: PROXY_URL
    headers: {
...
async function invite(discord_token, invite_code) {
  ...
  let response = await fetch("https://orange-term-46ed.snwioug.workers.dev/?url=" + target, { // 変更点3: PROXY_URL
    "headers": {
      "x-debug-options": "bugReporterEnabled",
      "x-discord-locale": new Intl.Locale(navigator.language).baseName,
      "x-discord-timezone": Intl.DateTimeFormat().resolvedOptions().timeZone,
...
async function invite_main(discord_token, invite_code, x_context_properties, x_fingerprint, session_id, hcaptcha_session_id, hcaptcha_rqtoken, hcaptcha_key) {
  ...
  const response = await fetch("https://orange-term-46ed.snwioug.workers.dev/?url=" + target, { // 変更点4: PROXY_URL
    headers: headers,
    body: JSON.stringify({
